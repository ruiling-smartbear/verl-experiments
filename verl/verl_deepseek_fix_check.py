# verl DeepSeek fix: does the branch that carries the synthetic tool-call
# arguments as a JSON string render tool appends on the real DeepSeek V3-family
# tokenizers, and does it leave every other builder's token ids untouched?
#
# Upstream main (3dab856, the #7628 merge) vs the fix branch, driven through the
# public tokenize_non_assistant_incremental_messages API on trajectories shaped
# exactly like tool_agent_loop's. Same harness as verl_fused_check.py.
import json
import os
import subprocess
import sys
...
MAIN_SHA = "3dab856da5eef3a5d20bc219371a9bcbf1aaa5a6"  # upstream main, the #7628 merge
# The fix commit on the fork (branch fix/deepseek-synthetic-tool-arguments); applied
# on top of upstream main for the "after" side, which is what the PR merge would be.
FIX_PATCH = "https://github.com/ruiling-smartbear/verl/commit/6543449935b0729732258318b86bfc89a85befda.patch"
...
def checkout(path: str, sha: str, patch_url: str | None = None) -> str:
    if not os.path.isdir(os.path.join(path, "verl")):
        ...
        if patch_url:
            subprocess.run(f"cd {path} && curl -sL {patch_url} -o /tmp/fix.patch && git apply --index /tmp/fix.patch",
                           shell=True, check=True)
...
def main() -> int:
    ...
    before = checkout("/tmp/verl_main_ds", MAIN_SHA)
    after = checkout("/tmp/verl_fix_ds", MAIN_SHA, FIX_PATCH)
    print(f"[fix] upstream main @{before} vs main @{after}, transformers {transformers.__version__}, "
          f"tool_agent_loop-shaped trajectories, 24 cases per row")
    for family, model, variants in FAMILY_MODELS:
        for variant in variants:
            old = run("/tmp/verl_main_ds", family, model, variant)
            if isinstance(old, str):
                print(f"[skip] {family} {model} {variant}: {old}")
                continue
            new = run("/tmp/verl_fix_ds", family, model, variant)
            ...
            print(
                f"[FIX]   {family:<9} {model:<30} {variant:<12} thinking_switch={new['has_thinking_switch']} "
