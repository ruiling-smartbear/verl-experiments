# verl DeepSeek follow-ups: with the tool-outputs prefix fix applied on top of
# current main (which already has #7630), do the V3 / R1 tool appends after the
# first tool turn match the template's full-conversation render, and does every
# other row stay byte-identical? Same harness as verl_fused_check.py.
import json
import os
import subprocess
import sys
...
MAIN_SHA = "f97625b751aadc6f683555a003de116ba20b458b"  # upstream main right after #7630
PATCHES = "https://raw.githubusercontent.com/ruiling-smartbear/sglang-omni/bench/975/patches/"
FIX_PATCHES = [
    PATCHES + "0001-rollout-fix-DeepSeek-tool-outputs-after-the-first-to.patch",
    PATCHES + "0001-rollout-feat-tool-parser-for-the-DeepSeek-V3-family.patch",
]
...
def checkout(path: str, sha: str, patch_urls: list[str] | None = None) -> str:
    if not os.path.isdir(os.path.join(path, "verl")):
        ...
        for index, patch_url in enumerate(patch_urls or []):
            subprocess.run(
                f"cd {path} && curl -sL '{patch_url}?t={index}{os.getpid()}' -o /tmp/fix{index}.patch "
                f"&& git apply --index /tmp/fix{index}.patch",
                shell=True, check=True,
            )
...
def main() -> int:
    ...
    before = checkout("/tmp/verl_main_fu", MAIN_SHA)
    after = checkout("/tmp/verl_fix_fu", MAIN_SHA, FIX_PATCHES)
    print(f"[followup] upstream main @{before} vs main @{after}, transformers {transformers.__version__}, "
          f"tool_agent_loop-shaped trajectories, 24 cases per row")
    for family, model, variants in FAMILY_MODELS:
        for variant in variants:
            old = run("/tmp/verl_main_fu", family, model, variant)
            if isinstance(old, str):
                print(f"[skip] {family} {model} {variant}: {old}")
                continue
            new = run("/tmp/verl_fix_fu", family, model, variant)
            ...
            print(
                f"[FU]    {family:<9} {model:<30} {variant:<12} thinking_switch={new['has_thinking_switch']} "
    ...
    print("DEEPSEEK_FOLLOWUP_CHECK_DONE")
    return 0
